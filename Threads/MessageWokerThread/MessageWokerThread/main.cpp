...
unsigned int WINAPI WorkerThread(void* /*lpParam*/)
{
    ...
    HANDLE waits[2] = { g_ExitEvent, g_WorkerEvent };

    for (;;)
    {
        DWORD wr = WaitForMultipleObjects(2, waits, FALSE, INFINITE);

        if (wr == WAIT_OBJECT_0)       // exitEvent
            return 0;

        if (wr != WAIT_OBJECT_0 + 1)   // workerEvent가 아닌 경우
            continue;
...
unsigned int WINAPI MoniteringThread(void* /*lpParam*/)
{
    while (true)
    {
        DWORD rrr = WaitForSingleObject(g_ExitEvent, 0);
        if (rrr == WAIT_OBJECT_0)
            return 0;

        DWORD r = WaitForSingleObject(g_TODO, 1000);
...
int main()
{
    ...
    g_TODO = CreateEvent(NULL, true, false, NULL);
